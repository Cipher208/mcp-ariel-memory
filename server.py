...
class MemoryMCPServer:
    ...
    def user_remember(self, user_id: str = "default", key: str = "", value: str = "", importance: float = 0.5, **kw) -> dict:
        entry_id = asyncio.run(self.mm.user_memory(user_id).remember(key, value, importance))
        return {"status": "ok", "entry_id": entry_id}

    def user_recall(self, user_id: str = "default", query: str = "", limit: int = 10, **kw) -> dict:
        results = asyncio.run(self.mm.user_memory(user_id).recall(query, limit))
        return {"results": results, "count": len(results)}

    def user_forget(self, user_id: str = "default", key: str = "", **kw) -> dict:
        deleted = asyncio.run(self.mm.user_memory(user_id).forget(key))
        return {"deleted": deleted}

    def user_session_start(self, user_id: str = "default", **kw) -> dict:
        session_id = asyncio.run(self.mm.user_memory(user_id).l2.create_session(user_id))
        return {"session_id": session_id}

    def user_session_end(self, user_id: str = "default", session_id: str = "", summary: str = "", **kw) -> dict:
        asyncio.run(self.mm.user_memory(user_id).l2.close_session(session_id, summary))
        return {"status": "ok"}

    def user_episode_save(self, user_id: str = "default", summary: str = "", weight: float = 0.5, tags: list = None, **kw) -> dict:
        episode_id = asyncio.run(self.mm.user_memory(user_id).l3.save(user_id, summary, weight, tags))
        return {"episode_id": episode_id}

    def user_episode_recall(self, user_id: str = "default", tag: str = "", limit: int = 10, **kw) -> dict:
        if tag:
            episodes = asyncio.run(self.mm.user_memory(user_id).l3.search_by_tag(user_id, tag, limit))
        else:
            episodes = asyncio.run(self.mm.user_memory(user_id).l3.get_episodes(user_id, limit))
    ...
    def user_graph_add(self, user_id: str = "default", content: str = "", node_type: str = "fact", tags: list = None, **kw) -> dict:
        node_id = asyncio.run(self.user_graph.add_node(user_id, content, node_type, tags))
        return {"node_id": node_id}

    def user_graph_query(self, user_id: str = "default", tag: str = "", node_type: str = "", limit: int = 20, **kw) -> dict:
        if tag:
            nodes = asyncio.run(self.user_graph.query_by_tag(user_id, tag, limit))
        elif node_type:
            nodes = asyncio.run(self.user_graph.query_by_type(user_id, node_type, limit))
        else:
            nodes = []
    ...
    def user_stats(self, user_id: str = "default", **kw) -> dict:
        ...
        return {
            ...
            "l2_sessions": asyncio.run(mem.l2.count_sessions(user_id)),
            "l3_episodes": len(asyncio.run(mem.l3.get_episodes(user_id, 1000))),
            "l4_facts": asyncio.run(mem.l4.count(user_id)),
            "wiki_pages": asyncio.run(self.user_wiki.count(user_id)),
            "graph_nodes": asyncio.run(self.user_graph.count_nodes(user_id)),
        }

    # === Agent Layer Tools ===

    def agent_remember(self, user_id: str = "default", key: str = "", value: str = "", importance: float = 0.5, **kw) -> dict:
        entry_id = asyncio.run(self.mm.agent_memory(user_id).remember(key, value, importance))
        return {"status": "ok", "entry_id": entry_id}

    def agent_recall(self, user_id: str = "default", query: str = "", limit: int = 10, **kw) -> dict:
        results = asyncio.run(self.mm.agent_memory(user_id).recall(query, limit))
        return {"results": results, "count": len(results)}

    def agent_forget(self, user_id: str = "default", key: str = "", **kw) -> dict:
        deleted = asyncio.run(self.mm.agent_memory(user_id).forget(key))
        return {"deleted": deleted}

    def agent_session_start(self, user_id: str = "default", **kw) -> dict:
        session_id = asyncio.run(self.mm.agent_memory(user_id).l2.create_session(user_id))
        return {"session_id": session_id}

    def agent_session_end(self, user_id: str = "default", session_id: str = "", summary: str = "", **kw) -> dict:
        asyncio.run(self.mm.agent_memory(user_id).l2.close_session(session_id, summary))
        return {"status": "ok"}

    def agent_episode_save(self, user_id: str = "default", summary: str = "", weight: float = 0.5, tags: list = None, **kw) -> dict:
        episode_id = asyncio.run(self.mm.agent_memory(user_id).l3.save(user_id, summary, weight, tags))
        return {"episode_id": episode_id}

    def agent_episode_recall(self, user_id: str = "default", tag: str = "", limit: int = 10, **kw) -> dict:
        if tag:
            episodes = asyncio.run(self.mm.agent_memory(user_id).l3.search_by_tag(user_id, tag, limit))
        else:
            episodes = asyncio.run(self.mm.agent_memory(user_id).l3.get_episodes(user_id, limit))
        return {"episodes": [{"id": e.episode_id, "summary": e.summary, "weight": e.emotional_weight} for e in episodes]}

    def agent_graph_add(self, user_id: str = "default", content: str = "", node_type: str = "decision_log", tags: list = None, **kw) -> dict:
        node_id = asyncio.run(self.agent_graph.add_node(user_id, content, node_type, tags))
        return {"node_id": node_id}

    def agent_graph_query(self, user_id: str = "default", tag: str = "", node_type: str = "", limit: int = 20, **kw) -> dict:
        if tag:
            nodes = asyncio.run(self.agent_graph.query_by_tag(user_id, tag, limit))
        elif node_type:
            nodes = asyncio.run(self.agent_graph.query_by_type(user_id, node_type, limit))
        else:
            nodes = []
        return {"nodes": [{"id": n.node_id, "content": n.content, "type": n.node_type, "tags": n.tags} for n in nodes]}
    ...
    def agent_stats(self, user_id: str = "default", **kw) -> dict:
        ...
        return {
            ...
            "l3_episodes": len(asyncio.run(mem.l3.get_episodes(user_id, 1000))),
            "l4_facts": asyncio.run(mem.l4.count(user_id)),
            "wiki_pages": asyncio.run(self.agent_wiki.count(user_id)),
            "graph_nodes": asyncio.run(self.agent_graph.count_nodes(user_id)),
        }
    ...
    def get_context(self, user_id: str = "default", layer: str = "user") -> str:
        return asyncio.run(self.mm.get_layer(layer, user_id).get_context())
