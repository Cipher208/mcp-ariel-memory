...
class AgentWiki:
    ...
    async def list_all(self, user_id: str, limit: int = 50) -> list[AgentWikiEntry]:
        conn = await self._cm.get("memory.db")
        cur = await conn.execute(
            "SELECT * FROM agent_wiki WHERE user_id=? ORDER BY updated_at DESC LIMIT ?", (user_id, limit)
        )
        rows = await cur.fetchall()
        return [self._row_to_entry(r) for r in rows]
    ...
    async def count(self, user_id: str = None, wiki_type: str = None) -> int:
        ...
        cur = await conn.execute(f"SELECT COUNT(*) FROM agent_wiki{where}", params)
        row = await cur.fetchone()
    ...
    async def _find_by_source(self, user_id: str, source: str) -> int | None:
        conn = await self._cm.get("memory.db")
        cur = await conn.execute("SELECT entry_id FROM agent_wiki WHERE user_id=? AND source=?", (user_id, source))
        row = await cur.fetchone()
