"""Tests for wiki/ module (FileWiki)."""
import sys
...
def test_file_wiki_add_search():
    ...
    path = w.add("work_notes", "Test Entry 2026", "Test content here", tags=["test"])
    assert path is not None
    assert Path(path).exists()
    results = w.search("Test Entry 2026")
...
def test_file_wiki_count():
    from wiki.file_wiki import FileWiki
    w = FileWiki(layer="user")
    w.add("work_notes", "Count Test 2026", "content")
    assert w.count() >= 1
