...
async def _insert(cm, user_id, key, value, imp, kind, days_ago, expires_in=None):
    ...
    await conn.execute(
        """INSERT INTO core_memory
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        (user_id, key, value, imp, kind,
         (now + expires_in * 86400) if expires_in else None,
         now - days_ago * 86400, now - days_ago * 86400),
    )
    await conn.commit()
...
async def test_decay_never_touches_instruction(cm):
    await _insert(cm, "u", "rule1", "никогда не удаляй бэкапы",
                  0.95, "instruction", days_ago=1000)
...
async def test_archive_handles_expired_goal(cm):
    await _insert(cm, "u", "g", "выучить Rust",
                  0.8, "goal", days_ago=10, expires_in=-1)
    fs = ForgettingSystem(cm=cm)
    n = await fs.archive_old_entries()
    assert n == 1
    conn = await cm.get("memory.db")
    row = await (await conn.execute(
        "SELECT 1 FROM archived_memories WHERE memory_type='goal'"
    )).fetchone()
