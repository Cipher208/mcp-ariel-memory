...
def test_engine_does_not_expose_legacy_methods():
    ...
    for name in ("search_rrf", "search_its", "search_binary"):
        assert not hasattr(e, name), \
            f"{name}() should be deleted, but RAGEngine still has it"


def test_facade_public_methods_only():
    """Public API = only search() with strategy param."""
    expected_public = {"search", "ingest_text", "ingest_file",
                       "init_db", "get_relations", "add_relation",
                       "count_pages", "count_chunks"}
    actual_public = {m for m in dir(RAGEngine)
                     if not m.startswith("_")
                     and callable(getattr(RAGEngine, m, None))}
    ...
    assert search_methods == set(), \
        f"unexpected search_* methods leaked: {search_methods}"
